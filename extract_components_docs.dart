...
class ComponentDocumentationExtractor {
  ...
  Future<void> _extractComponentFromFile(File file) async {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('class ') && line.contains('Naked') && !line.contains('_')) {
        ...
        if (className != null && className.startsWith('Naked')) {
          final component = ComponentInfo(
            ...
            filePath: file.path,
            constructorParams: await _extractConstructorParams(content, className),
  ...
  Future<List<String>> _extractConstructorParams(String content, String className) async {
    final lines = content.split('\n');
    final params = <String>[];
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (line.contains('$className(') || (line.contains(className) && line.contains('('))) {
        inConstructor = true;
      ...
      if (inConstructor) {
        ...
        if (line.contains('this.') || line.contains('required') || line.contains('@')) {
  ...
  List<String> _extractFeatures(String docComment) {
    ...
    for (final line in lines) {
      ...
      } else if (trimmed.contains('provides') || trimmed.contains('supports') || trimmed.contains('handles')) {
  ...
  String generateMarkdown() {
    ...
    buffer.writeln();
    buffer.writeln('This document contains comprehensive documentation for all Naked UI components.');
    ...
    for (final component in components) {
      buffer.writeln('- [${component.name}](#${component.name.toLowerCase().replaceAll('naked', '').trim()})');
    }
    buffer.writeln();
  ...
  void _writeComponentSection(StringBuffer buffer, ComponentInfo component) {
    ...
    buffer.writeln();
    final relativePath = component.filePath.replaceAll(RegExp(r'^.*naked_ui/'), '');
