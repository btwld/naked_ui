...
`NakedAccordion` gives you expandable sections without dictating layout or visuals. Pair it with `NakedAccordionController` to enforce how many panels can open, then render the trigger/header via the typed state snapshot.
  ...
  Widget build(BuildContext context) {
    return NakedAccordion<String>(
      ...
      builder: (context, accordionState) {
        ...
        final bool isHovered = accordionState.isHovered;
...
- Legacy interaction callbacks: `onFocusChange`, `onHoverChange`, `onPressChange` (prefer `builder` pattern to access `accordionState.isHovered`, etc.)
...
- Provide clear focus styles through `accordionState.isFocused`
