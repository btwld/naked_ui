...
- `states` → immutable hover/focus/pressed/disabled state set captured from the group container
...
- `states` → immutable `Set<WidgetState>` to resolve hover, focus, press
...
The controller is caller-owned; dispose it with the owning state.
...
- `excludeSemantics` omits only the trigger semantics contributed by
  `NakedAccordion`; panel semantics remain available
