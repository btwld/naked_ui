...
<CodeGroup title="Icon Button with Semantic Tooltip" defaultLanguage="dart">
```dart
import 'package:flutter/material.dart';
import 'package:naked_ui/naked_ui.dart';
...
Called when the button is tapped or activated via keyboard. The control remains
interactive when only `onLongPress` is provided, but keyboard activation requires
`onPressed`.
...
Omits only the button semantics contributed by `NakedButton`. Semantics from the
builder or child remain in the tree.
