...
- `hasValue` helper plus the immutable `states` set from the trigger builder

### Options – `NakedSelectOptionState<T>`
- `value` → option value
- `isSelected` → convenience getter supplied via `WidgetState.selected`
- `states` → immutable hover/focus/pressed/disabled state set
...
- `overlayBuilder` → `(BuildContext, RawMenuOverlayInfo)` – build any list/panel/search UI
- `value` / `onChanged` → manage the selected value externally (falls back to internal state when `value` is null)
- `positioning` → tune `OverlayPositionConfig` anchor alignment and offset for the dropdown shell
...
- `semanticLabel` → replace the trigger's derived accessible name
- `semanticValue` / `semanticValueFormatter` → announce the selected value;
  string values are announced automatically, while arbitrary objects are never
  converted with `toString()` implicitly
- `excludeSemantics` → omit only Naked UI's trigger semantics; option semantics remain available
...
Use `NakedSelectOption<T>` directly. `NakedSelect.Option(...)` is also available
...
Use the builder to react to `optionState.isSelected` or
`optionState.states` for hover styling. Options respect the select's `enabled`
and `closeOnSelect` settings automatically.
