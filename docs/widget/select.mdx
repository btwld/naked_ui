...
- `closeOnSelect` → close immediately after choosing an option
- `enabled` → disables trigger & option activation when false
- `semanticLabel` → override trigger semantics; `value?.toString()` is announced automatically otherwise
- `triggerFocusNode` → reuse an external focus node to regain focus after close
