...
`NakedSelect` mirrors a native select/dropdown experience—keyboard navigation, focus trapping, semantics—while leaving visuals entirely up to you. Provide a trigger builder (or child), render an overlay, and compose options with `NakedSelect.Option`.
