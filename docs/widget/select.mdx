...
`NakedSelect` mirrors a native select/dropdown experience—keyboard navigation, focus trapping, semantics—while leaving visuals entirely up to you. Provide a trigger builder, render an overlay, and compose options with `NakedSelect.Option`.
  ...
  Widget build(BuildContext context) {
    return NakedSelect<String>(
      ...
      triggerBuilder: (context, selectState) {
        final bool isOpen = selectState.isOpen;
        final bool hovered = selectState.isHovered;
        final bool focused = selectState.isFocused;
        ...
        return Container(
          ...
          child: Row(
            ...
            children: [
              Text(selectState.value ?? 'Select fruit'),
  ...
  Widget build(BuildContext context) {
    return NakedSelect.Option<String>(
      ...
      builder: (context, optionState, _) {
        final bool isSelected = optionState.isCurrentSelection;
...
### Options – `NakedSelectItemState<T>`
- `value` → option value
- `selectedValue` → value selected in the surrounding select
- `isCurrentSelection` → shorthand boolean
...
- `triggerBuilder` → `(BuildContext, NakedSelectState<T>)`
- `overlayBuilder` → `(BuildContext, RawMenuOverlayInfo)` – build anything (list, grid, search field)
- `value` / `onChanged` → manage selected value externally
- `positioning` → tune `OverlayPositionConfig` alignment, fallback, and offset for the dropdown shell
- `closeOnSelect` / `closeOnClickOutside` → opt into immediate dismissal after selection and pointer taps outside
...
- Lifecycle hooks: `onOpen`, `onClose`, `onCanceled` fire around overlay visibility; `onOpenRequested`, `onCloseRequested` let you intercept RawMenuAnchor requests (e.g. for animations)
...
Use the builder to react to `optionState.isCurrentSelection` or `optionState.widgetStates` for hover styling.
