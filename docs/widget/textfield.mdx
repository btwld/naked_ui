...
- Use `error`, `semanticErrorText`, and `semanticValidationResult` to expose
  validation state without relying on color alone
- `NakedTextField` wraps the builder result with proper `Semantics` for focusable text fields
- `excludeSemantics` removes only the wrapper semantics and leaves the editable
  text's native semantics intact
