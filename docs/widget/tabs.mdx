...
- `excludeSemantics` → omit only Naked UI's tab semantics while preserving descendant semantics
...
- Tabs expose tab semantics within a tab-list role; Tab/Shift+Tab enter or leave the group
- Arrow keys move and select within the group; Home/End select the first/last tab
  (horizontal or vertical behavior follows `orientation`)
