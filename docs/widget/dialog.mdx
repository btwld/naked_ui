...
<Info>
  [Browse the complete example](https://github.com/btwld/naked_ui/blob/main/packages/example/lib/api/naked_dialog.0.dart).
</Info>
...
- modal: When true, scopes the semantic route and blocks background semantics.
- semanticLabel: Optional screen reader label.
- semanticsRole: Defaults to `SemanticsRole.dialog`; use
  `SemanticsRole.alertDialog` for a message that requires an immediate response.
- excludeSemantics: Omits only the semantics contributed by `NakedDialog` while
  preserving semantics from `child`.

## Notes

- A dismissible barrier requires a localized `barrierLabel`; the function throws
  `ArgumentError` when one is omitted.
- `NakedDialog` only contributes semantics; visuals are entirely up to you.
