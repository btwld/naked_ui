...
- `isHovered`, `isFocused`, `isPressed`, `isDisabled` → inherited from `NakedState`
- `states` → immutable `Set<WidgetState>` if you need manual resolution
...
- `excludeSemantics` → omit only Naked UI's toggle semantics while preserving descendant semantics
