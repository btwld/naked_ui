...
void main() {
  ...
  group('NakedTooltip Integration Tests', () {
    testWidgets('tooltip shows on hover and hides on exit', (tester) async {
      ...
      await tester.pump(const Duration(milliseconds: 350)); // Wait for animation + removal
    ...
    testWidgets('tooltip respects wait duration', (tester) async {
          ...
          body: Center(
            child: NakedTooltip(
              ...
              tooltipBuilder: (context) => Container(
                ...
                child: const Text('Tooltip', style: TextStyle(color: Colors.white)),
              ),
              child: Container(
                ...
                child: const Text('Hover Target', style: TextStyle(color: Colors.white)),
              ),
            ),
          ),
        ),
      ...
      addTearDown(gesture.removePointer);
      await tester.pump(const Duration(milliseconds: 200)); // Wait less than waitDuration
    ...
    testWidgets('tooltip respects show duration', (tester) async {
          ...
          body: Center(
            child: NakedTooltip(
              ...
              child: Container(
                ...
                child: const Text('Show Duration Target', style: TextStyle(color: Colors.white)),
              ),
            ),
          ),
        ),
      ...
      // Exit hover
      await gesture.moveTo(const Offset(0, 0));
      await tester.pump(const Duration(milliseconds: 100)); // Wait less than showDuration
          ...
          body: Center(
            child: NakedTooltip(
              ...
              tooltipBuilder: (context) => Container(
                ...
                child: const Center(child: Text('Above', style: TextStyle(color: Colors.white))),
              ),
              child: Container(
                width: 80,
                height: 30,
                ...
                child: const Center(child: Text('Target', style: TextStyle(color: Colors.white))),
              ),
            ),
          ),
        ),
    ...
    testWidgets('tooltip onStateChange callback works', (tester) async {
      ...
      // Hide tooltip
      await gesture.moveTo(const Offset(0, 0));
      await tester.pump(const Duration(milliseconds: 250)); // Wait for showDuration
    ...
    testWidgets('tooltip handles rapid hover events correctly', (tester) async {
      ...
      await gesture.moveTo(tester.getCenter(targetFinder));
      await tester.pump(const Duration(milliseconds: 250)); // Wait for waitDuration
        ...
        home: Scaffold(
          body: Center(
            child: NakedTooltip(
              waitDuration: Duration.zero,
              showDuration: Duration.zero,
              tooltipSemantics: 'This is tooltip information',
    ...
    testWidgets('tooltip works with example app animation', (tester) async {
      ...
      await gesture.moveTo(const Offset(0, 0));
      await tester.pump(const Duration(milliseconds: 350)); // Wait for removalDelay + animation
      expect(find.text('This is a tooltip'), findsNothing);
    });
  });
}