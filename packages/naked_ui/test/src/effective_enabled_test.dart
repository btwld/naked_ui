...
void main() {
  group('Effective Enabled Behavior', () {
    ...
    testWidgets('NakedTabs disables when onChanged is null', (tester) async {
      // Track selection state via builder
      ...
      expect(tab2Selected, isFalse, reason: 'Tab 2 should not be initially selected');

      // Try to tap Tab 2 - it should not change selection since no callback
      await tester.tap(find.text('Tab 2'));
      await tester.pump();

      // Verify Tab 1 is still selected (selection didn't change)
      expect(tab1Selected, isTrue, reason: 'Tab 1 should remain selected after tap');
      expect(tab2Selected, isFalse, reason: 'Tab 2 should not become selected');
    });
  });
}
