...
All A1–A6, B1–B6, and C1–C3 implementation tasks are present in the working
tree after maintainer approval of D-12–D-15. The inventory guard, keyboard
postcondition, tab-order assertion, golden 1px mutation, and unnamed-target
accessibility failure were each proven to fail for the intended reason; all
temporary mutations were restored.

Local verification passed on Flutter 3.41.0 and 3.41.2, real macOS, and pinned
Chrome/ChromeDriver 150.0.7871.115. The inspected Dialog screenshot is an
800×600 real-macOS PNG with a complete manifest. Flutter 3.41.2 web screenshots
remain explicitly unsupported after both the WebDriver path and a repaint-
boundary fallback proved unstable; the blocking web behavior gate uploads its
test log instead. No Android emulator/AVD is installed locally, so Android and
the GitHub trigger/release-gate wiring require their first remote run after the
...
- Flutter 3.41.2: format and analyze clean; 574 package tests passed with 3
  documented skips; 11 example tests passed.
