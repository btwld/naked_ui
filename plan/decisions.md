...
### Component-plan decision evidence (2026-07-13)

These resolutions come from the clean-sheet component review requested on
2026-07-13. The review compared current Naked UI source/tests and open PRs with
Flutter 3.41.0/3.41.2, Flutter 3.44.6 current-stable source, beta/master
canaries, official Flutter package implementations, current `shadcn_flutter`
source, and relevant WAI-ARIA/WCAG behavior. Decisions that depend on real
assistive-technology output remain open rather than being settled from static
API research. The exact upstream audits are recorded in
...
- **D-16:** `Semantics.linkUrl` is platform-affecting, not metadata-only.
  Disabled Link output omits it. Native web navigation is proven with an
  optional official `url_launcher.Link` composition; Naked UI gains no core
  launcher dependency by default.
...
|---|---|---|---|---|
| D-01 | Toggle option semantics migration (`selected` → `toggled`) | Preserve `selected` for existing single mode; use `toggled` only for future multiple mode; route Tabs/Radio intent correctly | Before Toggle Group implementation (phase 4) | [resolved(split semantics by mode; multiple consumer-gated)](#component-plan-decision-evidence-2026-07-13) |
...
| D-04 | Toast composition API | Structured message/action/close helpers so duplicate message semantics can be excluded without hiding controls | Before Toast tests are written (phase 6) | [resolved(structured message/action/close)](#component-plan-decision-evidence-2026-07-13) |
| D-05 | Toast global shortcut | Caller opt-in only; canonical example may use F8; never reserve a key by default | Before Toast PR approval (phase 6) | [resolved(caller opt-in only)](#component-plan-decision-evidence-2026-07-13) |
| D-06 | Toast queue overflow | Unlimited pending queue or explicit nullable `maxQueued`; never drop silently without a dismissal reason | Before controller implementation (phase 6) | [resolved(one visible FIFO; nullable cap with explicit overflow reason)](#component-plan-decision-evidence-2026-07-13) |
| D-07 | Toast swipe in first release | Defer unless all alternate dismissal + deterministic drag tests fit the PR | At Toast scoping (phase 6) | [resolved(defer from MVP)](#component-plan-decision-evidence-2026-07-13) |
| D-08 | Field/TextField duplicate metadata | Debug-assert conflicting values; allow identical values; field scope is semantic source of truth | Before Field implementation (phase 3) | [resolved(field source; identical accepted, conflict asserts)](#component-plan-decision-evidence-2026-07-13) |
| D-09 | Initial Field error announcement | Initial error discoverable but not automatically assertive; later transitions announce once | Before Field semantics tests (phase 3) | [resolved(initial discoverable; later changes announce once)](#component-plan-decision-evidence-2026-07-13) |
...
| D-16 | Link destination and native-browser contract | Treat `linkUrl` as platform-affecting; omit when disabled; prove optional `url_launcher.Link` composition without a default core dependency | Before Link PR approval (phase 2) | [resolved(platform-affecting URI plus optional official adapter)](#component-plan-decision-evidence-2026-07-13) |
| D-17 | Hover Card raw overlay primitive | Prefer per-instance `RawMenuAnchor`/existing positioning; use `RawTooltip` only if per-instance close/focus/Escape is proven | During Hover Card spike (phase 7) | [resolved(spike menu anchor first; RawTooltip reference only)](#component-plan-decision-evidence-2026-07-13) |
| D-18 | Combobox raw foundation and first mode | Spike `RawAutocomplete`; single editable selection first; multiple/token selection separate | Before Combobox API work (phase 8) | [resolved(unexported RawAutocomplete single-select spike)](#component-plan-decision-evidence-2026-07-13) |
| D-19 | Flutter raw-primitive release policy | Public APIs at the minimum only; current stable is compatibility evidence; beta/master are canaries; no `package:flutter/src` or experimental feature dependency | Every phase contract review | [resolved(stable/floor policy plus release watchlist)](#component-plan-decision-evidence-2026-07-13) |
