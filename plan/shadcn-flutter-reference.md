...
| Link | `Button.link` is a visual button variant built on the generic `Clickable`; both Enter and Space activate it, and it does not establish link role/destination semantics. | Treat it only as a visual-state example. It cannot replace `NakedLink`; retain one link node, Enter-only keyboard activation, disabled destination removal, and the optional `url_launcher.Link` adapter. |
