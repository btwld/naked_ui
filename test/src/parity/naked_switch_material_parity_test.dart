...
void main() {
  group('Material Parity - Switch', () {
    ...
    testWidgets('enabled/disabled transitions parity (semantics)', (
      ...
      bool valueMaterial = false;
      bool valueNaked = false;
            ...
            return Row(
              ...
              children: [
                Switch(
                  key: materialKey,
                  value: valueMaterial,
                  onChanged: enabledMaterial
                      ? (v) => setState(() => valueMaterial = v)
                      : null,
                ),
                NakedSwitch(
                  key: nakedKey,
                  ...
                  onChanged: (v) => setState(() => valueNaked = v ?? false),
                  child: const SizedBox(width: 48, height: 24),
                ),
                TextButton(
                  onPressed: () => setState(() {
      ...
      // Disabled: assert Naked robustly
      expect(
        tester.getSemantics(find.byKey(nakedKey)),
        matchesSemantics(
          hasToggledState: true,
    ...
    testWidgets('semantics parity (checked state and actions)', (tester) async {
      const materialKey = Key('material');
      const nakedKey = Key('naked');
      ...
      await tester.pumpMaterialWidget(
        Row(
          ...
          spacing: 24,
          children: [
            Switch(key: materialKey, value: false, onChanged: (_) {}),
            NakedSwitch(
              key: nakedKey,
              value: false,
              onChanged: (_) {},
