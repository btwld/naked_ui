...
class AnchoredOverlayShell extends StatelessWidget {
  ...
  static void _defaultOnOpenRequested(Offset? position, VoidCallback show) =>
      show();

  static void _defaultOnCloseRequested(VoidCallback hide) => hide();
  ...
  Widget build(BuildContext context) {
    return RawMenuAnchor(
      ...
      overlayBuilder: (context, info) {
        ...
        final top =
            (useTop ? (info.anchorRect.top) : (info.anchorRect.bottom)) +
            offset.dy;

        // Clamp left/top into the overlay's bounds to avoid going negative.
        final clampedLeft = left.clamp(0.0, info.overlaySize.width);
        final clampedTop = top.clamp(0.0, info.overlaySize.height);
